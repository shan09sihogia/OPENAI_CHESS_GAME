...
""")
        ...
        if st.button("Start Game"):
            ...
            st.info("Now, the AI agents will compete with one another.  The board will be examined by each agent, " 
                   "request legal moves from the Game Master (proxy agent), and make strategic decisions.")
            st.success("After the agent turns are over, you can see all of the chess board moves shown below, as well as the interaction between the agents in the terminal output!")
            st.write("The game began!  White's turn.")
        ...
        if st.button("Reset Game"):
            ...
            st.session_state.move_history = []
            st.session_state.board_svg = None
            st.write("Reset the game!  To start a new game, click 'Start Game'.")

    except Exception as e:
        st.error(f"An error occurred: {e}. Please try again after checking your API key.")

else:
    st.warning("To begin the game, please input your OpenAI API key in the sidebar.")